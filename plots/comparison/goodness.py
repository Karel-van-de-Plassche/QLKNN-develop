...
def find_similar_goodness(network_id):
    ...
    similar = [(res.hyperparameters.get().goodness , res.id) for res in query]
    similar.sort()
    labels = [(line[1], 'goodness = ' + line[0] + '') for line in similar]
    print('nn_list = OrderedDict([', end='')
    print(*labels, sep=',\n', end='')
    print('])', end='')

find_similar_goodness(37)
